package br.ederlima.ui 
{
	...
	public dynamic class ListItem extends MovieClip 
	{
		...
		private var _duration:TextField = new TextField();
		private var _playBtn:TextField = new TextField();
		private var _dividerTxt:TextField = new TextField();
		private var _iconVisited:IconVisited = new IconVisited();
		...
		private var _width:Number = 850;
		private var _backgroundColor:uint = 0;
		private var _colorString:String = "";
		private var _alternateColor:uint = 0;
		private var _alternateColorString:String = "";
		
		private var _defaultFormat:TextFormat = new TextFormat("Arial", "11", 0xF0F0F0);
		private var _statusFormat:TextFormat = new TextFormat("Arial", "10", 0xD7A544);
		private var _durationFormat:TextFormat = new TextFormat("Arial", "10", 0x65BDCF);
		private var _playFormat:TextFormat = new TextFormat("Arial", "10", 0x686868);
		 ...
		 */
		public function ListItem(type:String = "", itemStyle:ListItemStyle = null, folderStyle:FolderItemStyle = null,data:Object = null) 
		{
			_type = type;
			...
			//_bg.graphics.drawRect(0, 0, _width, 15);
			_bg.graphics.drawRect(-50, 0, _width, 15);
			_bg.graphics.endFill();
			_bg.name = "bg";
			this.addChild(_bg);
			
			_labelField.mouseEnabled = _idField.mouseEnabled = false;
			_labelField.x = 80;
			//_labelField.x = 26;
			_labelField.y = _idField.y =  5;
			_labelField.antiAliasType = _idField.antiAliasType = _statusTxt.antiAliasType = _duration.antiAliasType = AntiAliasType.ADVANCED;
			_playBtn.antiAliasType = _dividerTxt.antiAliasType = AntiAliasType.ADVANCED;
			_labelField.width = 400;
			_idField.width = 80;//_width - 36;
			_labelField.multiline = true;
			_labelField.wordWrap = true;
			_labelField.autoSize = _idField.autoSize = _statusTxt.autoSize = _duration.autoSize = TextFieldAutoSize.LEFT;
			_playBtn.autoSize =  _dividerTxt.autoSize = TextFieldAutoSize.LEFT;
			_labelField.defaultTextFormat = _idField.defaultTextFormat = _defaultFormat;
			...
			_playBtn.x = 785;
			_iconVisited.x = 817;
			_statusTxt.y = _duration.y = _dividerTxt.y = _playBtn.y = _iconVisited.y = 5
				
			
			
			_statusTxt.defaultTextFormat = _statusFormat;
			_duration.defaultTextFormat = _durationFormat;
			_dividerTxt.defaultTextFormat = _playFormat;
			_playBtn.defaultTextFormat = _playFormat;
			_playBtn.selectable = false;
			_playBtn.mouseEnabled = true;

			_statusTxt.text = "Not viewed";
			_duration.text = "4 min";
			_playBtn.htmlText = "<a href='event:null'>Play</a>";
			_dividerTxt.text = "|";
			...
			//this.addChild(_dividerTxt);
			//this.addChild(_iconVisited);
			
			_idField.x = 10;
			if(data != null)	{
				this.data = data;
				_idField.text = data.slNo;
			...
			_hit.graphics.drawRect(0, 0, _width, 15);
			...
			//this._playBtn.addEventListener(MouseEvent.CLICK, clickHandler);
			this._playBtn.addEventListener(TextEvent.LINK,clickHandler);
			
			//trace(":::::::::::::::::"+this.height)
			this.x =-5;
		}
		public function setAttempted():void	{
			_statusTxt.text = isVisited ? "" : "Not viewed";
			_iconVisited.gotoAndStop(isVisited ? 1 : 2);
			
			if(data.widget == "true" )	{
				_iconVisited.gotoAndStop(3);
			}
			
			
			if(_type == ListItemType.TYPE_FOLDER)	{
				_iconVisited.visible = false;
				_statusTxt.text = "";
				_playBtn.text = "";
				_dividerTxt.text = "";
			}
		}
		private function clickHandler(event:TextEvent):void 	{
			this.dispatchEvent(new ListItemEvent(ListItemEvent.LIST_ITEM_CLICKPLAY, this as ListItem));
		}
		...
		{
			_width = value;
			_bg.width = 900//_width;
			_hit.width = 900//_width;
		...
		{
			_label = value.length > 100 ? value.substring(0,100)+"..." : value;;
